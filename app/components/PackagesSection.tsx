      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
        {packages.map((pkg) => {
          ...
          if (pkg.id === 'highland'){return (
            <EventsPackageCard pkg = {pkg} isSelected={isSelected} onSelect={onSelect}/>

          )}else{
            return (
              <StandardPackageCard pkg = {pkg} isSelected={isSelected} onSelect={onSelect}/>
            )
          }
...
const StandardPackageCard = ({ pkg, isSelected, onSelect }: { pkg: Package, isSelected: boolean, onSelect: (pkg:Package) => void }) => {
  return (
            <div
              key={pkg.id}
              className={`relative flex flex-col rounded-xl overflow-hidden border-2 transition-all duration-300
                ${pkg.highlight
...
const EventsPackageCard = ({ pkg, isSelected, onSelect }: { pkg: Package, isSelected: boolean, onSelect: (pkg:Package) => void }) => {
  return (
                <div className={`p-[2px] rounded-xl bg-linear-to-br from-cyan-400 via-teal-400
                 to-blue-600 shadow-lg  transition-all duration-300 
                 ${isSelected ? 'shadow-xl shadow-amber-500/20 scale-[1.02]' : ''}`}>

            <div
              key={pkg.id}
              className={`relative flex flex-col rounded-xl h-full overflow-hidden border-2 transition-all duration-300
                ${pkg.highlight
                  ? 'border-amber-500 shadow-xl shadow-amber-500/20 scale-105'
                  : 'border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500'
                }
